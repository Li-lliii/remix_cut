...
def _get_default_voice_path() -> Path:
    return PLATFORM_ROOT / "assets" / "default_voice" / "dongbei_clone_5s.wav"
...
def _get_replace_speech_workflow_path() -> Path:
    return Path(
        os.environ.get(
            "BS_MEDIA_REPLACE_SPEECH_WORKFLOW_PATH",
            str(PROJECT_ROOT / "workstream" / "ai_transforms" / "replace_speech_api.json"),
        )
    ).expanduser().resolve()
...
def submit_comfyui_job(
    *,
    ...
    command = [
        ...
        "--workflow-path",
        str(_get_replace_speech_workflow_path()),
    ]
